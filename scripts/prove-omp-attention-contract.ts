...
async function main(): Promise<void> {
  ...
  try {
    ...
    await writeFile(
      ...
      `import { assertOmpAttentionEvent, type OmpAttentionEvent } from "@tomismeta/aperture/omp-attention-event";\nconst input: OmpAttentionEvent = { schemaVersion: 4, type: "omp.attention-event", eventId: "event-1", occurredAt: "2026-09-01T16:00:00.000Z", sessionId: "session;opaque", session: { label: "release-check", facets: [{ id: "branch", label: "Branch", value: "main" }] }, interactionId: "tool-1", classification: "approval_requested", title: "OMP needs approval", summary: "OMP is waiting for an operator decision.", transition: "requested", focus: { kind: "opaque-focus", handle: "A23456789_-bcdefghijklmnopqrstuv" } };\nassertOmpAttentionEvent(input);\n`,
      "utf8",
    );
    await writeFile(
      join(consumerDir, "index.mjs"),
      `import assert from "node:assert/strict";\nimport { assertOmpAttentionEvent, resolveOmpAttentionSocketPath } from "@tomismeta/aperture/omp-attention-event";\nimport schema from "@tomismeta/aperture/omp-attention-event.schema.json" with { type: "json" };\nconst event = assertOmpAttentionEvent({ schemaVersion: 4, type: "omp.attention-event", eventId: "event-1", occurredAt: "2026-09-01T16:00:00.000Z", sessionId: "session;$(opaque)", session: { label: "release-check", facets: [{ id: "branch", label: "Branch", value: "main" }] }, interactionId: "tool-1", classification: "approval_requested", title: "OMP needs approval", summary: "OMP is waiting for an operator decision.", transition: "requested", focus: { kind: "opaque-focus", handle: "A23456789_-bcdefghijklmnopqrstuv" } });\nassert.equal(event.sessionId, "session;$(opaque)");\nassert.equal(event.session?.label, "release-check");\nassert.deepEqual(event.session?.facets, [{ id: "branch", label: "Branch", value: "main" }]);\nassert.equal(event.focus?.kind, "opaque-focus");\nassert.equal(schema.$id, "urn:tomismeta:aperture:omp-attention-event:v4");\nassert.equal(resolveOmpAttentionSocketPath({ XDG_RUNTIME_DIR: "/run/user/1000" }), "/run/user/1000/omarchy/aperture/attention.sock");\n`,
      "utf8",
    );
