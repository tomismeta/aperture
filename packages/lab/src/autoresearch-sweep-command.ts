...
import { AUTORESEARCH_SWEEP_STATUS_SCHEMA_VERSION } from "./artifact-versions.js";
import { acquireAutoresearchProcessLock } from "./autoresearch-lock.js";
import { ensureCleanRepo } from "./autoresearch-workspace.js";
import {
...
type AutoresearchSweepStatus = {
  schemaVersion: typeof AUTORESEARCH_SWEEP_STATUS_SCHEMA_VERSION;
  updatedAt: string;
  generatedAt: string;
  sweepId: string;
...
const SWEEP_STATUS_SCHEMA_VERSION = AUTORESEARCH_SWEEP_STATUS_SCHEMA_VERSION;
  ...
  try {
    ...
    for (const [laneIndex, lane] of lanes.entries()) {
      ...
      try {
        ...
        await logLine(
          logPath,
          `lane_finish label=${label} status=${result.status} final_status=${result.finalStatus}`,
        );
      } catch (error) {
        const preserved = await preserveAutoresearchSweepLaneArtifacts({
          laneRoot,
        ...
        await logLine(
          logPath,
          `lane_error label=${label} error=${sanitizeLogField(laneResult.error)}`,
        );
        await writeSweepStatus(statusPath, {
          generatedAt,
          sweepId,
...
export async function preserveAutoresearchSweepLaneArtifacts(options: {
  ...
  result?: Pick<
    ...
    | "selectedProposalPath"
    | "selectedPatchPath"
  >;
}): Promise<
  Omit<
    AutoresearchSweepLaneResult,
    "label" | "dataset" | "split" | "status" | "finalStatus" | "laneRoot" | "error"
  >
> {
  ...
  assignIfDefined(
    ...
    "reportPath",
    await copyIfPresent(
      options.result?.currentReportPath ??
        path.join(runtimeRoot, "current-campaign", "current-report.json"),
  ...
  assignIfDefined(
    ...
    "reportMarkdownPath",
    await copyIfPresent(
      options.result?.currentReportMarkdownPath ??
        path.join(runtimeRoot, "current-campaign", "current-report.md"),
  ...
  assignIfDefined(
    ...
    preserved.proposalPath
      ? await copyAdjacentMarkdown(
          options.result?.selectedProposalPath,
          path.join(options.laneRoot, "proposal.md"),
        )
      : undefined,
  );
  assignIfDefined(
...
async function copyAdjacentMarkdown(
...
function defaultLaneLabel(
  index: number,
  lane: Pick<AutoresearchSweepLane, "dataset" | "split">,
): string {
