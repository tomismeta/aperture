export function compileAssertionScope(value: string | readonly string[]) {
  ...
  const authoritativeFields = scopedFields.map((clauses) =>
    actual
      ? clauses.filter(
          (clause) =>
            (!NON_ASSERTED_SCOPE.test(clause) || CONCRETE_DIAGNOSTIC.test(clause)) &&
            !NON_ASSERTED_EVENT_FACT.test(clause),
        )
      : [],
  );
...
export function stripScopedClauses(value: string, owns: (clause: string) => boolean) {
  ...
  for (const clause of splitAssertions(value)) {
    if (/^(?:but|however|yet)\b/i.test(clause.trim())) suppress = false;
...
export const isTaskFailureTextFallbackSuppressed = (value: string): boolean =>
  ...
  (FAILURE_REF.test(value) && !FAILURE_REF.test(value.replace(QUOTED_TEXT, " quoted-content "))) ||
  NON_ASSERTED_SCOPE.test(lf(value)) ||
  frameIsNonAsserted(lf(value), false);
export const splitAssertions = (value: string): string[] => value.split(ASSERTION_BOUNDARY);
export const frameIsNonAsserted = (value: string, allowWrappers = true): boolean =>
  (allowWrappers || !value.trim().toLowerCase().startsWith("observation:")) &&
  NON_ASSERTED_FRAME.test(value.replace(/^(?:but|however|yet)\b[,:]?\s*/i, ""));
const lf = (v: string) => splitAssertions(v).reverse().find(FAILURE_REF.test, FAILURE_REF) ?? "";
