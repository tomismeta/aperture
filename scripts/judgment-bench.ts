...
import { defaultLabRuntimeSubdirectory, renderJudgmentBenchMarkdown, runJudgmentBench } from "@aperture/lab";
...
async function main(): Promise<void> {
  ...
  process.stdout.write(
    [
      ...
      `now lanes: ${result.summary.totalNowLanes}`,
      `next lanes: ${result.summary.totalNextLanes}`,
      `ambient lanes: ${result.summary.totalAmbientLanes}`,
      `results: ${jsonPath}`,
