...
test("@tomismeta/aperture-core exposes the intended public SDK surface", () => {
  ...
  assert.equal("looksLikeExplicitDiagnosticObservationTranscript" in sdk, false);
  ...
  assert.equal("looksLikeExplicitDiagnosticObservationTranscript" in semanticSdk, false);
...
test("public SDK applies semantic defaults to direct ApertureEvent publishes by default", () => {
  ...
  assert.equal(
    trace.eventTransition.changedFields.some(
      (field) =>
        field.path === "consequence" && field.before === undefined && field.after === "low",
    ),
    true,
  );
  assert.equal(
  ...
  assert.equal(
    trace.frameTransition.changedFields.some(
      (field) => field.path === "responseSpec.kind" && field.after === "approval",
    ),
    true,
  );
...
test("public SDK can opt out of semantic defaults for direct ApertureEvent publishes", () => {
  ...
  assert.equal(
    trace.frameTransition.changedFields.some(
      (field) => field.path === "consequence" && field.after === "medium",
    ),
    true,
  );
...
test("public SDK supports trace inspection through the trace subpath", () => {
  ...
  assert.equal(
    trace.eventTransition.changedFields.some(
      (field) => field.path === "semantic.intentFrame" && field.after === "question_request",
    ),
    true,
  );
  assert.equal(
  ...
  assert.equal(
    trace.frameTransition.changedFields.some(
      (field) => field.path === "mode" && field.after === "choice",
    ),
    true,
  );
  assert.equal(trace.semantic?.toolFamily, "read");
