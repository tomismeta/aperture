...
import { AUTORESEARCH_RETAINED_BACKLOG_SCHEMA_VERSION } from "./artifact-versions.js";
import type {
...
export { AUTORESEARCH_RETAINED_BACKLOG_SCHEMA_VERSION } from "./artifact-versions.js";
...
export function defaultAutoresearchRetainedBacklogMarkdownPath(filePath: string): string {
...
export async function updateAutoresearchRetainedBacklog(options: {
  ...
  const entries = new Map((existing?.entries ?? []).map((entry) => [entry.key, entry] as const));
  ...
  for (const attempt of options.run.retainedAttempts ?? []) {
    ...
    const merged: AutoresearchRetainedBacklogEntry = {
      key,
      focusArea:
        primarySignal?.focusArea ?? attempt.snapshot.intentStatements[0]?.focusArea ?? "unknown",
      ...
      gateBlockedCount:
        (current?.gateBlockedCount ?? 0) + Number(attempt.retainedOutcome === "gate_blocked"),
      optimizerCleanCount:
        (current?.optimizerCleanCount ?? 0) + Number(attempt.retainedOutcome === "optimizer_clean"),
...
export function renderAutoresearchRetainedBacklogMarkdown(
  ...
  if (backlog.entries.length === 0) {
    lines.push("", "No retained near-miss proposals recorded yet.");
    return `${lines.join("\n")}\n`;
  }
  ...
  for (const [index, entry] of backlog.entries.entries()) {
    ...
    lines.push(`${index + 1}. ${summaryLine}`);
    lines.push(
  ...
  for (const [index, entry] of backlog.entries.entries()) {
    ...
    if (latestOccurrence) {
      lines.push(
        ...
        "",
        ...(latestOccurrence.runMarkdownPath
          ? [`- Run review: ${latestOccurrence.runMarkdownPath}`]
          : []),
        ...(latestOccurrence.runPath ? [`- Run JSON: ${latestOccurrence.runPath}`] : []),
        ...(latestOccurrence.proposalPath
          ? [`- Proposal JSON: ${latestOccurrence.proposalPath}`]
          : []),
        ...(latestOccurrence.batchReportPath
          ? [`- Batch report: ${latestOccurrence.batchReportPath}`]
          : []),
        ...(latestOccurrence.optimizerRunPath
          ? [`- Optimizer run: ${latestOccurrence.optimizerRunPath}`]
          : []),
      );
    }
  }
...
function deriveRetainedAttemptKey(attempt: AutoresearchRunnerRetainedAttempt): string {
  const primarySignal = attempt.snapshot.signals[0];
...
function collectIntentTargets(intents: readonly AutoresearchProposalIntentStatement[]): string[] {
