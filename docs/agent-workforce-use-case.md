...
That means each source should have an ingress adapter or plugin that maps source-native events into `AdapterEvent`, which `@aperture/core` then normalizes semantically.
