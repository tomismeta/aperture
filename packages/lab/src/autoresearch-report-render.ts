...
export function renderAutoresearchFinalReportMarkdown(
  ...
  if (report.runSummary.workflow) {
    const workflowSummary = formatWorkflowTargetMetadataRollupSummary(report.runSummary.workflow);
    if (workflowSummary) {
      lines.push(`- workflow: ${workflowSummary}`);
    }
    const workflowUsage = formatWorkflowTargetMetadataRollupUsage(report.runSummary.workflow);
    if (workflowUsage) {
